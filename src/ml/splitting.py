...
def grouped_stratified_split(labels: Sequence[str], groups: Sequence[str], holdout: float,
    ...
    yüksek ölçülür (veri sızıntısı). Bu yüzden aynı gruptaki örnekler hep aynı tarafa düşer.
    Neden tabakalı: her grup en sık görüldüğü etikete atanır ve her etiket içinde grupların
    yaklaşık `holdout` oranı ayrılır; böylece konu dağılımı iki tarafta da korunur.
    """
    ...
    # Her grup, içinde en çok bulunan etiketin listesine konur. Eşitlikte alfabetik ilk
    # etiket seçilir (sorted), böylece sonuç her çalıştırmada aynıdır.
    ...
    for label in sorted(groups_by_label):
        ...
        # Bu etiket için ayrılması gereken örnek sayısı; hedefe ulaşana kadar grup eklenir.
